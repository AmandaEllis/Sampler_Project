...
for(j in 1:k){
    ...
    output<-new.X(X.MH[[i-1]],cap.occasion)
    ...
    #This is completed in 2 steps, we compute the densities of the proposal distributions and the CDL
    ...
    #n* is the number of available indodivuals after the removal of photograph
    ...
    X.current.proposal.density<-1/(output$n.ID.curr.X+1)
    X.candidate.proposal.density<-1/(output$n.ID.cand.X+1)
